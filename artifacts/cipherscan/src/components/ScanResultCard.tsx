  ...
  return (
    <>
      ...
      {isZoomOpen && scan.previewImageUrl && (
        ...
        >
          <div 
            className="relative max-w-5xl w-full bg-card border border-border/50 rounded-xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]"
          ...
          >
            <div className="p-4 border-b border-border/50 bg-black/40 flex items-center justify-between">
              <div className="flex items-center gap-2 font-mono text-xs text-muted-foreground truncate">
                ...
                <span className="truncate text-foreground font-medium">{scan.originalUrl}</span>
              </div>
              <button 
                onClick={() => setIsZoomOpen(false)}
